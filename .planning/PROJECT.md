...
- **TTS v1 engine roster (REQ-22):** `F5-TTS`, `XTTS v2`, `Qwen3-TTS 0.6B-Base`, and `VoxCPM2`. Qwen3-TTS is explicitly **not** the default: it failed the acceptance gate on TTFA/RTF and still lacks approved accent quality, but it remains included as an opt-in engine. VoxCPM2 is `selectable_with_caveats`: manual quality is far superior to F5, but current RayMe calls do not yet consume VoxCPM2 streaming chunks, so F5 remains the default for fastest call feel.
