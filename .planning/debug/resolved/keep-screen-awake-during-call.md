...
- timestamp: "2026-08-09T13:45:04Z"
  checked: "Canonical OMEN deployment"
  found: "`scripts/deploy-omen.sh` pulled and verified `dae560cb290c3ea3acef9b4ac190304f40e7b023`, rebuilt the web client, preserved the canonical scheduled-task launchers, restarted both services, verified ports 8443/9443, and completed successfully."
  implication: "The wake-lock implementation is deployed through the only authorized mechanism on the exact tested code commit."

- timestamp: "2026-08-09T13:46:10Z"
  checked: "Deployed OMEN mobile Chromium regression"
  found: "`npm --prefix web-ui\\client run test:e2e -- tests/e2e/call-mobile.spec.ts --project=mobile-chromium --reporter=line` passed 2/2 tests on OMEN, covering successful `request('screen')` after call start and a rejected request that shows the fallback notice while End Call remains usable."
  implication: "The deployed production route exercises both the granted and rejected wake-lock UI paths in a mobile browser profile."

- timestamp: "2026-08-09T13:46:58Z"
  checked: "Independent deployed identity and live-call readiness"
  found: "OMEN reports repository HEAD and `/webrtc/status.deployed_commit` as `dae560cb290c3ea3acef9b4ac190304f40e7b023`; WebRTC status is `ready` with `live_call_ready=true`, `media_transport_ready=true`, and `active_sessions=0`; web-to-AI readiness is `ready` and authenticated."
  implication: "The exact wake-lock commit is live and the call stack is ready for the product owner's physical phone acceptance test."

- timestamp: "2026-08-09T17:52:51Z"
  checked: "Product-owner physical Android phone acceptance"
  found: "The product owner tested the deployed behavior and reported: 'that works well. we can close this.'"
  implication: "The original real-device idle-timeout workflow is accepted as resolved on the deployed release."
...
- verification:
    ...
    deployed_browser_e2e: { result: pass, command: "npm --prefix web-ui\\client run test:e2e -- tests/e2e/call-mobile.spec.ts --project=mobile-chromium --reporter=line", assertions: "2/2 pass on OMEN" }
    human_acceptance: { result: pass, environment: "deployed Android phone", evidence: "Product owner reported: 'that works well. we can close this.'" }
    pending: "none"
- files_changed:
...
- 5_whys:
  - code_branch: "The call lifecycle never requested a screen wake lock because its resource model covered media and call state but not device-screen state; browser-lifecycle behavior was outside the existing call tests. The first implementation then made a completion-driven retry possible because a rejected request left an eligible visible call with no sentinel."
  - environment_branch: "Browser power, permission, or implementation policy can reject or automatically release a correct Screen Wake Lock request; that external condition is normal API behavior, but it exposed the code branch because the old `finally` path immediately retried every persistent denial."
  - and_gate: "The retry loop required both branches: the unconditional code retry and a persistent external rejection. The original idle timeout itself required only the missing code request."
- why_not_caught: "No existing unit, mobile browser, or physical-phone acceptance gate covered the Screen Wake Lock lifecycle, persistent rejected-request behavior, or a visible fallback notice."
- recurrence_guard: "`web-ui/client/tests/unit/call-wake-lock.test.ts` covers `bounds persistent rejections until a new visible lifecycle event requests again`; `web-ui/client/tests/e2e/call-mobile.spec.ts` covers successful acquisition and `explains when the browser cannot keep the screen awake without ending the call`; both passed before deployed Android acceptance."
