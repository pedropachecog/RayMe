...
- **next_action:** Stage only the Qwen recovery-sampler adapter, its exact request regressions, and this active debug record. Publish and deploy through the canonical OMEN script, then repeat the exact controlled-clone swipe test with at least 30 real action-route samples.
