...
- **Global scope**: `D:/Pedro/Repos/Program/RayMe/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/Pedro/Repos/Program/RayMe/.claude/notes/`)
...
2. Glob `D:/Pedro/Repos/Program/RayMe/.claude/notes/*.md` (if directory exists) — global notes
...
Global (D:/Pedro/Repos/Program/RayMe/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/Pedro/Repos/Program/RayMe/.claude/notes/` — works in any directory
